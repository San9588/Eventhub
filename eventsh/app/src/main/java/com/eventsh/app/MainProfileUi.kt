...
fun MainActivity.profileDialog(existing: Profile?) {
    ...
    fun refreshCtx() {
        ...
        if (contexts.isEmpty()) {
            ctxBox.addView(Maniflow.text(this, "(no triggers yet - add one)", 14f, t.textMuted).apply {
            ...
            for (i in contexts.indices) {
                val c = contexts[i]
                val idx = i
                ctxBox.addView(ctxRow(c.summary(), t.textPrimary) {
                    editContext(contexts, idx) { refreshCtx() }
                })
            }
        }
        ctxBox.addView(ctxRow("+ ADD TRIGGER", t.accentPrimary) {
